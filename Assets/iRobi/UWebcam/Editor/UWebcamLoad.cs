...
class UVPaintLoad : EditorWindow {
	...
	[MenuItem ("Window/iRobi/UWebcam (UNet Camera System)")]
	...
	public static void  ShowWindow () {
		...
		Afghan = Resources.Load ("UWebcamIcon") as Texture2D;
	...
	static UVPaintLoad()
	{
		string key = Application.dataPath + "UWebcamLoadFirstStart";
		if (!EditorPrefs.HasKey (key)) {
			ShowWindow ();
			Debug.Log ("UWebcam was successfully loaded.");
		}
	...
	void OnGUI () {
		...
		GUI.Label (rect6, "UWebcam:",style);
		GUI.Label (rect7, "Version 1.0",style);
